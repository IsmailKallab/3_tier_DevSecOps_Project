...
resource "aws_vpc" "DevSecOps_Project_vpc" {
  cidr_block = "10.0.0.0/16"

  tags = {
    Name = "DevSecOps_Project_vpc"
  }
}

resource "aws_subnet" "DevSecOps_Project_subnet" {
  count = 2
  vpc_id                  = aws_vpc.DevSecOps_Project_vpc.id
  cidr_block              = cidrsubnet(aws_vpc.DevSecOps_Project_vpc.cidr_block, 8, count.index)
  availability_zone       = element(["us-east-1a", "us-east-1b"], count.index)
  map_public_ip_on_launch = true

  tags = {
    Name = "DevSecOps_Project_subnet${count.index}"
  }
}
